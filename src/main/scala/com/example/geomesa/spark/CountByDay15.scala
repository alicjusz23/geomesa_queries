 ...
 ***********************************************************************/
/*
 * Counts, how many gtd are inside generalized urban areas
*/
 ...
 // Create DataFrame using the "geomesa" format
   ...
   val gtdDataFrame = sparkSession.read
    ...
    val shpDataFrame = sparkSession.read
      .format("geomesa")
      .options(shpParams)
      .option("geomesa.feature", "shp_eu_s_m")
      .load()
      shpDataFrame.createOrReplaceTempView("shp")

    val sqlQuery  = "create temporary view temp as (select * from gtd left join shp on st_contains(shp.the_geom, gtd.geom) where st_contains(shp.the_geom, gtd.geom) is not null)" 
